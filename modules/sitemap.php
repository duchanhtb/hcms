...
class sitemap extends Module {
    ...
    function draw() {
        ...
        $Product = new Product();
        $arrNumberProduct = $Product->countProduct();
        ...
        if (count($allCat) > 0) {
            ...
            foreach ($allCat as $key => $value) {
                ...
                $subCat = $Category->getSubCat($allCat, $value['id']);
                if (count($subCat) > 0) {
        ...
        $sitemap = Input::get('sitemap', 'int', 0);
        $html_other = '';
        if ($sitemap == 0) {
            ...
            $News = new News();
            $News->num_per_page = 10;
            $con = " AND status = 1 AND cat_id = $cid ";
            $arrNews = $News->getNews($con, 'ordering', 'desc', 1);
            if (count($arrNews) > 0) {
    ...
    function createSiteMap($map) {
        switch ($map) {
            case "index":
                ...
                $Category = new Category();
                $allCat = $Category->getCatByType(1);

                $News = new News();
                $News->num_per_page = 100;

                $con = ' AND status = 1';
                if (1) {
                    $list_subcat = $Category->getListSubCategory($allCat, 0);
                    $list_subcat = trim($list_subcat, ',');
                    $con .= " AND cat_id IN ($list_subcat) ";
                }
                $listNews = $News->getNews($con, 'ordering', ' DESC ', 1);
                if (count($listNews) > 0) {
            ...
            case "product":
                ...
                $listProduct = $Product->getProduct($con, 'ordering', ' DESC ', 1);
